import { Translations } from "../types/types";

export const translations: Translations = {
  en: {
    nav_link_one: "mission",
    nav_link_two: "clients",
    nav_link_three: "contact",
    footer_logo_text:
      "(noun)  / Part of a song recurring between verses  / Group of backing vocalists supporting the lead singer",

    home_text: "Connecting Asian Music and Culture with the World",
    mission_text: [
      "CHORUS is a new ecosystem dedicated to uplifting and connecting Asian music and culture with the world. ",
      "Founded by renowned Asian executives, CHORUS strives to support Asian artists and their teams to achieve true resonance worldwide. Our vision is to advance the representation and celebration of Asian music from across the continent and diaspora, and elevate its place on the global stage. ",
      "As a growing team of leaders and experts in music, CHORUS rallies the unmatched insight, expertise and passion of our alliance towards our mission – to champion Asian music and culture, and empower Asian artists, teams and labels to participate in the success, development and growth of Asian music around the world.",
      "Leveraging this wealth of expertise, Chorus is also uniquely equipped to support international acts from outside of Asia in building authentic and impactful connections with Asian audiences and cultures, and growing these into deep, long-lasting relationships in those markets.",
    ],
    contact_text: [
      "For further enquiries and to request additional details, please email:",
      "info@chorus.asia",
      "Follow us on Instagram and LinkedIn",
    ],
  },
  jp: {
    nav_link_one: "mission",
    nav_link_two: "clients",
    nav_link_three: "contact",
    footer_logo_text:
      "(noun)  / Part of a song recurring between verses  / Group of backing vocalists supporting the lead singer",

    home_text: "アジアの音楽と文化を世界と繋ぐ",
    mission_text: [
      "Chorusはアジアの音楽と文化を活気づけ、世界と結び付けることに特化した新たなエコシステム。",
      "数多くの実績で知られ、アジアにルーツを持つメンバーにより設立されたChorusは、アジア系アーティストとそのチームが世界中で真の共感を得るための支援に注力します。私たちのビジョンは、彼らが作り出す音楽の表現と祝福を促進させ、世界規模でその地位を高めることです。",
      "アジア音楽界のリーダーや専門家で構成され、規模を拡大させているChorusは、チーム一丸となって目指すミッションのために比類ない洞察力、専門知識、情熱を結集させます - 私たちのミッションは、アジアの音楽と文化のために闘い、アジア系アーティスト、チーム、レーベルをエンパワーし、世界中でアジア音楽の成功、発展、成長へ貢献することです。",
      "Chorusはその幅広い専門性を活かすことで、アジア圏外で活動する海外アーティストがアジアのオーディエンスやカルチャーと真摯でインパクトある繋がりを築くこと、そしてその繋がりをマーケット内で深く、長きに渡る関係へと成長させるための支援も行う。",
    ],
    contact_text: [
      "お問い合わせやその他詳細をご希望の方は",
      "info@chorus.asiaまでご連絡ください。",

      "Follow us on Instagram and LinkedIn.",
    ],
  },
};
